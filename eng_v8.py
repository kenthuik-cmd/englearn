...
# 📱 PWA / iPhone 图标支持
# =========================
# 关键点：page_icon 使用真正的本地图片，先于页面渲染设置。
# 这样 Streamlit Cloud 不依赖 iframe JavaScript，也能拿到自定义 favicon。
st.set_page_config(
...
# Streamlit 的静态文件服务由 .streamlit/config.toml 开启。
# 同时把 manifest / apple-touch-icon 注入到最外层页面，兼容 iPhone 添加到主屏幕。
components.html(
    """
    <script>
    (function () {
        ...
        const icon = origin + "/app/static/icon-192.png";
        const manifest = origin + "/app/static/manifest.json";
        ...
        function applyPWA() {
            ...
            // 把 Streamlit 的 favicon 也换成自己的图标，避免 iPhone / 浏览器回退到 Streamlit。
            let favicons = doc.querySelectorAll('link[rel="icon"], link[rel="shortcut icon"]');
            if (!favicons.length) {
            ...
            meta('theme-color', '#1677ff');
        }

        applyPWA();
        // Streamlit 可能在页面加载后重新生成 head 元素，因此短时间重复检查。
        let count = 0;
