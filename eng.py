...
def render_autoplay_study_component(queue, mastered_words, current_lvl):
    ...
    return render_autoplay_review_component(
        ...
        user_id=st.session_state.user.id if "user" in st.session_state else "solo_admin_888",
...
def render_autoplay_review_component(mastered_words, auto_mark=False, current_lvl=None, user_id=None, initial_mastered=None):
    ...
    <script>
      ...
      async function play(){const t=token;if(!running||!current)return;$('heroStatus').textContent="正在朗读…";await speak(current.word,"en-US",0.82,t);if(!running||t!==token)return;if($('zhPlay').checked&&current.meaning)await speak(current.meaning,"zh-CN",0.9,t);if(!running||t!==token)return;timer=setTimeout(()=>next(),550);}
      function next(){
        if(!running)return;
      ...
      function start(){if(!words.length)return;running=true;$('startScreen').style.display='none';$('playScreen').style.display='block';$('zhPlay').checked=$('zhStart').checked;index=-1;next();}
      function stop(){running=false;clean();$('heroStatus').textContent="已暂停";$('status').innerText=autoMark?"✓ 已保存当前进度":"";}
      $('startBtn').addEventListener('click',start);$('stopBtn').addEventListener('click',stop);$('zhPlay').addEventListener('change',()=>{});
