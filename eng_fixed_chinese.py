...
def render_autoplay_review_component(mastered_words, auto_mark=False, current_lvl=None, user_id=None, initial_mastered=None):
    ...
    <div class="app">
      ...
      <div id="playScreen" class="listen" style="display:none">
        ...
        <div class="card">
          ...
          <div class="status" id="status">正在准备…</div><button class="pause-btn" id="pauseBtn">⏸ 暂停</button><button class="stop-btn" id="stopBtn">⏹ 停止挂机听</button>
        </div>
    ...
    <script>
      ...
      let index=-1,running=false,paused=false,token=0,timer=null,current=null;
      ...
      async function saveProgress(){
        if(!autoMark)return false;
        const endpoint=supabaseUrl.replace(/\/$/,"")+"/rest/v1/user_profiles";
        try{
          const r=await fetch(endpoint,{method:"POST",headers:{"apikey":supabaseKey,"Authorization":"Bearer "+supabaseKey,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({user_id:userId,grade:currentLvl,mastered_words:mastered.join(",")})});
          if(!r.ok)throw new Error(r.status);
          $('status').innerText="✓ 已算作认识并保存";
          setTimeout(()=>{if($('status').innerText==="✓ 已算作认识并保存")$('status').innerText=""},900);
          return true;
        }catch(e){$('status').innerText="⚠️ 保存失败，请检查网络或 Supabase 权限";return false;}
      }
      ...
      function speechWord(text){return String(text||"").trim().replace(/^(?:(?:n|v|adj|adv|prep|conj|pron|num|art|aux|vt|vi|det|phr|phrase)\.?\s+)+/i,"").trim();}\n      function speechChinese(text){return String(text||"").trim().replace(/(^|[；;、\n])\s*(?:n|v|adj|adv|prep|conj|pron|num|art|aux|vt|vi|det|phr|phrase)\.?\s*/gi,"$1").replace(/(^|[；;、\n])\s*(?:名词|动词|形容词|副词|介词|连词|代词|数词|冠词|助动词)[:：]?\s*/g,"$1").trim();}
      ...
      function start(){if(!words.length)return;running=true;paused=false;$('startScreen').style.display='none';$('playScreen').style.display='block';$('zhPlay').checked=$('zhStart').checked;$('examplePlay').checked=$('exampleStart').checked;index=-1;next();}
      async function stop(){
        if(current&&autoMark){const w=String(current.word||"").trim();if(w&&!mastered.includes(w))mastered.push(w);await saveProgress();}
        running=false;paused=false;clean();$('pauseBtn').textContent="⏸ 暂停";$('heroStatus').textContent="已停止";
        if(autoMark){const url=new URL(window.parent.location.href);url.searchParams.set("lvl_"+currentLvl,mastered.join(","));url.searchParams.set("lvl",String(currentLvl));url.searchParams.set("exit_autoplay","1");window.parent.location.replace(url.toString());}
      }
      function pauseToggle(){
        if(!running)return;
        if(paused){paused=false;window.speechSynthesis.resume();$('pauseBtn').textContent="⏸ 暂停";$('heroStatus').textContent="正在朗读…";$('status').innerText="";}
        else{paused=true;window.speechSynthesis.pause();$('pauseBtn').textContent="▶ 继续";$('heroStatus').textContent="已暂停";$('status').innerText="已暂停，可继续播放";}
      }
            ...
            async function saveMasteredProgress() {{
                ...
                try {{
                    const response = await fetch(endpoint, {{
                        ...
                        body: JSON.stringify({{ user_id: userId, grade: currentLvl, mastered_words: masteredStr }})
                    }});
